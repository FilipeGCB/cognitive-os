#!/usr/bin/env python3
"""Run resumable Cognitive OS V1.5 behavioral conformance through a remote provider.

The runner uses only Python stdlib and an explicit remote chat-completions
provider adapter. SUT execution and grading are separate, independently
checkpointed phases. A SUT response is cached by the case/package/eval/model/
provider/config request identity; grader identity is intentionally absent from
that key, so changing graders does not rerun the SUT. No local-model transport
or implicit provider is supported.
"""
...
RUNNER_SCHEMA = "cognitive-os-conformance-v4"
SUT_ARTIFACT_SCHEMA = "cognitive-os-sut-execution-v2"
...
DEFAULT_OUTPUT = "/tmp/cognitive-os-v1.5-conformance.json"
LOCAL_PROVIDER_NAMES = {"ollama", "local", "loopback", "embedded"}
LOOPBACK_HOSTS = {"localhost", "localhost.localdomain", "127.0.0.1", "::1"}
CRITICAL_IDS = {
...
class ProviderUnavailable(RuntimeError):
    """Raised when a provider was not explicitly configured or cannot run."""
...
class ProviderAdapter(Protocol):
    ...
    def observe_identity(self, model: str) -> dict[str, object]:
        ...
...
class UnavailableProvider:
    """Provider object used to make missing configuration explicit and inert."""

    configured = False
    status = "UNAVAILABLE"

    def __init__(self, reason: str, provider_name: str | None = None, base_url: str | None = None):
    ...
    def chat(self, model: str, messages: list[dict], *, config: dict[str, object]) -> tuple[str, dict]:
        del model, messages, config
        raise ProviderUnavailable(self.reason)


def _provider_url(provider_name: str, base_url: str) -> tuple[urllib.parse.SplitResult, str]:
    if provider_name.strip().lower() in LOCAL_PROVIDER_NAMES:
        raise ValueError("local providers are not allowed for behavioral conformance")
    parsed = urllib.parse.urlsplit(base_url.strip())
    hostname = (parsed.hostname or "").lower()
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise ValueError("provider base_url must be an absolute HTTP(S) URL")
    if parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise ValueError("provider base_url must not embed credentials, query secrets or fragments")
    if hostname in LOOPBACK_HOSTS or hostname.endswith(".localhost") or hostname.startswith("127."):
        raise ValueError("loopback provider endpoints are not allowed for behavioral conformance")
    path = parsed.path.rstrip("/")
    if path.endswith("/chat/completions"):
        endpoint = urllib.parse.urlunsplit((parsed.scheme, parsed.netloc, path, "", ""))
    else:
        endpoint_path = f"{path}/chat/completions" if path.endswith("/v1") else f"{path}/v1/chat/completions"
        endpoint = urllib.parse.urlunsplit((parsed.scheme, parsed.netloc, endpoint_path, "", ""))
    return parsed, endpoint


class RemoteChatProvider:
    """Small provider-neutral adapter for an explicit remote chat transport."""

    configured = True
    status = "AVAILABLE"

    def __init__(self, provider_name: str, base_url: str, api_key: str, *, timeout: int = DEFAULT_REQUEST_TIMEOUT):
        parsed, endpoint = _provider_url(provider_name, base_url)
    ...
    def api_root(self) -> str:
        return self.endpoint.rsplit("/chat/completions", 1)[0]
    ...
    def _request(self, url: str, *, data: bytes | None = None) -> dict:
        ...
        if not isinstance(body, dict):
            raise ValueError("remote provider returned a non-object JSON response")
        return body

    def observe_identity(self, model: str) -> dict[str, object]:
        model_path = urllib.parse.quote(model, safe="")
        try:
            body = self._request(f"{self.api_root}/models/{model_path}")
            observed_model = str(body.get("id") or body.get("model") or "")
            identity_material = {
                "provider": self.provider_name,
                "model": model,
            ...
            fingerprint = stable_hash(identity_material)
            return {
                "provider": self.provider_name,
                "name": model,
                "model": model,
    ...
    def chat(self, model: str, messages: list[dict], *, config: dict[str, object]) -> tuple[str, dict]:
        ...
        body = self._request(self.endpoint, data=json.dumps(payload).encode("utf-8"))
        choices = body.get("choices")
        if not isinstance(choices, list) or not choices or not isinstance(choices[0], dict):
            raise ValueError("remote provider response has no choice")
        message = choices[0].get("message")
        if not isinstance(message, dict):
            raise ValueError("remote provider response has no message")
        content = message.get("content")
        if isinstance(content, list):
            content = "".join(str(item.get("text") or "") for item in content if isinstance(item, dict))
        if not isinstance(content, str):
            raise ValueError("remote provider response content is not text")
        metadata = dict(body)
        metadata["_requested_max_tokens"] = max_tokens
        metadata["_provider"] = self.provider_name
        metadata["_endpoint"] = self.endpoint
...
def build_provider(
    *,
    ...
    """Build only an explicitly configured remote provider; never select a fallback."""

    if not provider_name or not base_url or not api_key:
        missing = [
            label for label, value in (("provider", provider_name), ("base_url", base_url), ("credential", api_key))
            if not value
        ]
        return UnavailableProvider(f"no provider configuration: missing {', '.join(missing)}", provider_name, base_url)
    return RemoteChatProvider(provider_name, base_url, api_key, timeout=timeout)


def provider_identity_observed(identity: object) -> bool:
    """Return true only for a host/provider-observed, release-bindable identity."""

    if not isinstance(identity, dict):
        return False
    fingerprint = str(identity.get("fingerprint") or identity.get("digest") or "")
    provider = str(identity.get("provider") or "").strip().lower()
    return bool(
        identity.get("observed") is True
        and provider
        and provider not in LOCAL_PROVIDER_NAMES
        and str(identity.get("name") or identity.get("model") or "").strip()
...
def request_config(case: dict, context_window: int, *, kind: str) -> dict[str, object]:
    if kind == "sut":
        return {
            ...
            "max_tokens": response_max_tokens_for(case["tags"]),
        }
    return {
        "temperature": 0,
        "context_window": context_window,
...
def response_max_tokens_for(tags: list[str]) -> int:
...
def response_flags(response: str, metadata: dict, *, tags: list[str], prompt: str = "") -> dict[str, object]:
    ...
    choices = metadata.get("choices") if isinstance(metadata.get("choices"), list) else []
    first_choice = choices[0] if choices and isinstance(choices[0], dict) else {}
    done_reason = str(metadata.get("done_reason") or first_choice.get("finish_reason") or "").lower()
    requested = metadata.get("_requested_max_tokens")
...
def execute_grade_case(
    ...
    *,
    ...
    if sut_entry.get("status") != "COMPLETE" or not isinstance(sut_entry.get("response"), str):
        ...
        try:
            raw_grade, metadata = provider.chat(
                grader_model,
                [
    ...
    result: dict[str, object] = {
        **descriptor,
        ...
        "grader_identity": grader_identity,
        "grader_independent": (
            str(grader_identity.get("provider") or ""),
            grader_model,
        ) != (
            str(sut_entry.get("model_identity", {}).get("provider") or ""),
            str(sut_entry.get("model_identity", {}).get("name") or ""),
        ),
...
def run_case(
    ...
    *,
    ...
    if not grader_model:
        return {"id": case.get("id", "UNKNOWN"), "status": "UNAVAILABLE", "overall": "UNAVAILABLE", "pass": False, "reason": "grader model must be explicitly configured"}
    candidate = candidate_sha()
    suite = "compat"
    eval_hash = case_contract_hash(case)
    skill_fingerprint = skill_package_fingerprint(candidate)
    identity = provider.observe_identity(model)
    if not getattr(provider, "configured", True) or not provider_identity_observed(identity):
        return {"id": case.get("id", "UNKNOWN"), "status": "UNAVAILABLE", "overall": "UNAVAILABLE", "pass": False, "reason": "SUT provider identity unavailable"}
    ...
    grader_identity = provider.observe_identity(grader_model)
    if not provider_identity_observed(grader_identity):
        return {"id": case.get("id", "UNKNOWN"), "status": "UNAVAILABLE", "overall": "UNAVAILABLE", "pass": False, "reason": "grader provider identity unavailable"}
...
def sut_header(
    *,
    ...
    return {
        ...
        "sut_model": model,
        "sut_provider": provider_name,
        "sut_provider_status": provider_status,
...
def run_grade_selection(
    *,
    ...
    header = {
        ...
        "sut_provider": sut_header_document.get("sut_provider", "UNKNOWN"),
        "sut_provider_status": sut_header_document.get("sut_provider_status", "UNKNOWN"),
        "sut_model_identity": sut_header_document.get("sut_model_identity", {}),
        "grader_provider": provider.provider_name,
        "grader_provider_status": provider.status,
        "grader_model": grader_model,
        ...
        "grader_independent": (
            ...
            str(sut_header_document.get("sut_provider", "")),
            str(sut_header_document.get("sut_model", "")),
        ),
        "context_window": context_window,
    }
    checkpoint_expected = {key: header[key] for key in header}
...
def build_report(
    *,
    ...
    incomplete_case_ids = sorted({str(case["id"]) for case in selected_cases} - result_ids)
    ...
    sut_identity_observed = provider_identity_observed(header.get("sut_model_identity"))
    grader_identity_observed = provider_identity_observed(header.get("grader_model_identity"))
    identities_independent = (
        str(header.get("sut_provider") or ""),
        str(header.get("sut_model") or ""),
    ) != (
        str(header.get("grader_provider") or ""),
        str(header.get("grader_model") or ""),
    )
    grader_independent = header.get("grader_independent") is True and identities_independent
    release_eligible = bool(
        complete
        and selection_complete
        and selection.get("critical_coverage_complete")
        and sut_identity_observed
        and grader_identity_observed
        and grader_independent
    ...
    if status in {"NOT_EXECUTED", "UNAVAILABLE"}:
        overall = "UNAVAILABLE"
    elif not complete or not selection_complete:
        overall = "INCOMPLETE"
    else:
        overall = "PASS" if release_eligible else "FAIL"
    flags = [result.get("flags", {}) if isinstance(result.get("flags"), dict) else {} for result in results]
    report = {
        ...
        **header,
        ...
        "provider_status": {
            "sut": header.get("sut_provider_status", "UNKNOWN"),
            "grader": header.get("grader_provider_status", "UNKNOWN"),
        },
        "provider_identity_observed": {
            "sut": sut_identity_observed,
            "grader": grader_identity_observed,
        },
        "grader_independent": grader_independent,
        ...
        "limitations": [
            "This run proves behavior only for the declared remote provider/model and prompt packaging.",
            "SUT execution and grading are separate phases; changing the grader must not rerun a matching cached SUT response.",
            "The grader is independent evidence only when the provider/model pair differs from the SUT pair and both identities were observed.",
            ...
            "No local-model transport or implicit provider fallback is supported.",
        ],
        "results": results,
        "flags_summary": {
...
def build_unavailable_report(
    *,
    ...
    """Create a non-PASS artifact without attempting an unconfigured call."""
    ...
    header = {
        ...
        "skill_package_fingerprint": skill_fingerprint,
        "eval_rubric_hash": eval_hash,
        "sut_provider": sut_provider or "UNCONFIGURED",
        "sut_provider_status": "UNAVAILABLE",
        "sut_model": sut_model or "UNCONFIGURED",
        "sut_model_identity": sut_identity,
        "grader_provider": grader_provider or "UNCONFIGURED",
        "grader_provider_status": "UNAVAILABLE",
        "grader_model": grader_model or "UNCONFIGURED",
        "grader_model_identity": grader_identity,
        "grader_independent": False,
        "context_window": context_window,
    }
    ...
    report["phase_requested"] = phase
    report["unavailable_reason"] = reason
    report["provider_configuration"] = {
        "sut": {"provider": sut_provider or None, "model": sut_model or None},
        "grader": {"provider": grader_provider or None, "model": grader_model or None},
        "fallback": None,
    }
    return report
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--provider", default=os.environ.get("COGNITIVE_OS_CONFORMANCE_PROVIDER"), help="Explicit remote SUT provider name")
    parser.add_argument("--base-url", default=os.environ.get("COGNITIVE_OS_CONFORMANCE_BASE_URL"), help="Explicit remote SUT base URL")
    parser.add_argument("--api-key-env", default=os.environ.get("COGNITIVE_OS_CONFORMANCE_API_KEY_ENV"), help="Environment variable containing the SUT provider credential")
    parser.add_argument("--model", default=os.environ.get("COGNITIVE_OS_CONFORMANCE_MODEL"), help="Explicit SUT model")
    parser.add_argument("--grader-provider", default=os.environ.get("COGNITIVE_OS_GRADER_PROVIDER"), help="Explicit remote grader provider name")
    parser.add_argument("--grader-base-url", default=os.environ.get("COGNITIVE_OS_GRADER_BASE_URL"), help="Explicit remote grader base URL")
    parser.add_argument("--grader-api-key-env", default=os.environ.get("COGNITIVE_OS_GRADER_API_KEY_ENV"), help="Environment variable containing the grader credential")
    parser.add_argument("--grader-model", default=os.environ.get("COGNITIVE_OS_GRADER_MODEL"), help="Explicit grader model")
...
def _credential_from_env(name: str | None) -> str | None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    sut_artifact_stats: dict[str, int] | None = None
    sut_provider: ProviderAdapter | None = None
    sut_provider_name = str(args.provider or "")
    sut_model = str(args.model or "")
    ...
    sut_model = str(sut_document.get("sut_model") or sut_model) if sut_document else sut_model
    if sut_document:
        sut_provider_name = str(sut_document.get("sut_provider") or sut_provider_name)
        if isinstance(sut_document.get("sut_model_identity"), dict):
            sut_identity = sut_document["sut_model_identity"]
    if args.phase in {"all", "sut"}:
        try:
            sut_provider = build_provider(
                provider_name=args.provider,
                base_url=args.base_url,
                api_key=_credential_from_env(args.api_key_env),
            )
        except ValueError as exc:
            sut_provider = UnavailableProvider(str(exc), args.provider, args.base_url)
        if not sut_model or not sut_provider.configured:
            sut_identity = sut_provider.observe_identity(sut_model)
            report = build_unavailable_report(
                ...
                grader_provider=str(args.grader_provider or ""),
                grader_model=grader_model,
                grader_identity={},
                context_window=args.context_window,
                reason=sut_provider.reason if not sut_provider.configured else "SUT model must be explicitly configured",
                phase=args.phase,
            )
            atomic_write_json(out, report)
            print(f"CONFORMANCE {report['overall']}: provider unavailable; model_calls=0")
            return 2
        sut_identity = sut_provider.observe_identity(sut_model)
        if not provider_identity_observed(sut_identity):
            report = build_unavailable_report(
                suite=args.suite,
                selection=selection,
                selected_cases=selected_cases,
                ...
                skill_fingerprint=skill_fingerprint,
                eval_hash=eval_hash,
                sut_provider=sut_provider.provider_name,
                sut_model=sut_model,
                sut_identity=sut_identity,
                grader_provider=str(args.grader_provider or ""),
                grader_model=grader_model,
                grader_identity={},
                context_window=args.context_window,
                reason="SUT provider identity could not be observed; refusing unbound execution",
                phase=args.phase,
            )
            atomic_write_json(out, report)
            print(f"CONFORMANCE {report['overall']}: SUT identity unavailable; model_calls=0")
            return 2
    ...
    if not provider_identity_observed(sut_identity):
        report = build_unavailable_report(
            ...
            grader_identity={},
            context_window=args.context_window,
            reason="SUT evidence has no observed provider/model identity; refusing grading",
            phase=args.phase,
        )
        atomic_write_json(out, report)
        print(f"CONFORMANCE {report['overall']}: SUT identity unavailable; model_calls=0")
    ...
    try:
        grader_provider = build_provider(
            provider_name=args.grader_provider,
            base_url=args.grader_base_url,
            api_key=_credential_from_env(args.grader_api_key_env),
        )
    except ValueError as exc:
        grader_provider = UnavailableProvider(str(exc), args.grader_provider, args.grader_base_url)
    if not grader_model or not grader_provider.configured:
        grader_identity = grader_provider.observe_identity(grader_model)
        report = build_unavailable_report(
            suite=args.suite,
            selection=selection,
            selected_cases=selected_cases,
            ...
            grader_identity=grader_identity,
            context_window=args.context_window,
            reason=grader_provider.reason if not grader_provider.configured else "grader model must be explicitly configured",
            phase=args.phase,
        )
        atomic_write_json(out, report)
        print(f"CONFORMANCE {report['overall']}: grader unavailable; model_calls=0")
        return 2
    grader_identity = grader_provider.observe_identity(grader_model)
    if not provider_identity_observed(grader_identity):
        report = build_unavailable_report(
            ...
            sut_model=sut_model,
            sut_identity=sut_identity,
            grader_provider=grader_provider.provider_name,
            grader_model=grader_model,
            grader_identity=grader_identity,
            context_window=args.context_window,
            reason="grader provider identity could not be observed; refusing unbound grading",
            phase=args.phase,
        )
        atomic_write_json(out, report)
        print(f"CONFORMANCE {report['overall']}: grader identity unavailable; model_calls=0")
        return 2
    ...
    if report["overall"] == "PASS":
        return 0
    return 2 if report["overall"] == "UNAVAILABLE" else 1
